names.state = ["C_A","T"];
names.input = ["q_plus","q_minus","T_c"];
names.param = [ ...
   "V",      ...  % reactor volume                   [L]
...
% Construct q from slack variables:
q = q_ref + q_plus - q_minus;
...
Tss = casadi.SX.sym('Tss');
qss = casadi.SX.sym('qss');
...
%%%%% The reference 
eq.state = [CA_ss; Tss];
eq.input = [q_plus; q_minus; T_c_ss];
...
model.style.input.q_plus.unit   = "[L/s]";
model.style.input.q_plus.label = "$q^{+}$";

model.style.input.q_minus.unit   = "[L/s]";
model.style.input.q_minus.label = "$q^{-}$";
